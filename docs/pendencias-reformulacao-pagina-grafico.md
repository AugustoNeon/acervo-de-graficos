...
> **Escopo**: registro detalhado de uma discussão de produto/design — o que foi decidido, o que já foi implementado, e o que falta. **Leia se**: for continuar o retrofit dos READMEs restantes, ou entender por que a "interativo"/datas sumiram do site numa sessão anterior. **O padrão de página em vigor agora é o descrito aqui** — `WORKFLOW.md` (passo 6) já foi atualizado pra refletir as duas zonas, não é mais preciso ler os dois.
...
7. **Retrofit dos 55 READMEs já publicados** — em andamento, aos poucos, cada um com commit(s) próprio(s) (critério confirmado pelo usuário). Progresso: **1/55** — [matriz-adjacencia-tags](../graficos/network/matriz-adjacencia-tags) feito nesta sessão, como piloto (verificado com `astro check`, build de produção e screenshot real via Chromium — as duas zonas renderizam como esperado). Restam 54.
8. **Escolher os links de "Gráficos parecidos"** pra cada um dos 55 restantes, à mão — trabalho editorial genuíno, não mecanizável (a régua "nunca por tag em comum" é justamente contra automatizar isso). Exemplo já feito: matriz de adjacência → rede densa hairball (oposto direto) + correlograma (mesma técnica, outro domínio).
9. **Revisitar o risco do item 5 das decisões** (duas fichas kraft competindo) depois que 3-4 gráficos tiverem bastidor de verdade lado a lado — só aí dá pra julgar se incomoda na prática. Ainda só 1 gráfico tem bastidor até agora, cedo pra avaliar.
