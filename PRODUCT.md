...
- **Toda página de gráfico segue as mesmas duas zonas, nessa ordem**: uma zona de **referência** (ficha técnica → o que é/pra que serve → quando usar e evitar → que dados precisa [condicional] → como ler → como foi feito → possíveis problemas → variações → gráficos parecidos [opcional]) e, opcionalmente, uma zona de **bastidor** ("Notas do coletor") com a história real de 1 decisão/bug do gráfico. As duas zonas nunca se misturam no mesmo tom — quem só quer a referência técnica consegue parar de ler antes do bastidor começar. Padrão detalhado no passo 6 do [`docs/WORKFLOW.md`](docs/WORKFLOW.md). Consistência entre páginas é o que separa "acervo" de "pasta de exemplos"; a audiência de referência é sempre alguém que pode ter chegado direto por um link, sem ter navegado o resto do site.
- **Texto escrito pra quem chega de fora**, nunca pra nós mesmos — sem "detectei", "tive dificuldade", "reaproveitei do gráfico anterior". **Exceção deliberada**: dentro de "Notas do coletor", processo e decisão real têm permissão de aparecer — é a única seção onde o bastidor pode ser mostrado, contado com intenção, nunca como diário de bordo cru.
